...
\paragraph{Example.} $\mu(n) = 2^{-n}$ is an example of a negligible function. Figure~\ref{fig:negligible} illustrates how $2^{-n}$ decays exponentially and is eventually smaller than any polynomial bound $n^{-c}$ for sufficiently large $n$. This can be observed as follows.
...
\begin{enumerate}
\item Sample $v \leftarrow \mathbb{QR}_N$ (such a $v$ can be obtained by sampling a random value in $\mathbb{Z}_N^*$ and squaring it) and compute $g := v^2 \mod N$. Note that $g$ is a generator of $\mathbb{QR}_N$ with high probability, since $v$ is a random quadratic residue and squaring preserves the cyclic structure.
...
\end{enumerate}
To understand why this works, observe that if $\mathcal{A}$ correctly computes the CDH value, then the returned values $u = g^{(x+ 2^{-1})(y + 2^{-1})} = v^{2xy + x+ y + 2^{-1}}$. Here, $2^{-1}$ denotes the multiplicative inverse of $2$ modulo the order of the group, which exists since the order is odd (as $\phi(N) = (p-1)(q-1)$ and both $\frac{p-1}{2}$ and $\frac{q-1}{2}$ are prime, implying $p$ and $q$ are odd). Consequently, the computed value $w = v^{2^{-1}}$. This follows from: $w = \frac{u}{g^{xy} \cdot v^{x+y}} = \frac{v^{2xy + x + y + 2^{-1}}}{v^{2xy + x + y}} = v^{2^{-1}}$. Furthermore, with probability $\frac12$ we have that $w \neq v$. This is because $w^2 = (v^{2^{-1}})^2 = v$, so $w$ is a square root of $v$. In $\mathbb{Z}_N^*$ where $N = pq$, each quadratic residue has exactly 4 square roots. Two of these are $\pm v$ (which don't help us factor), and the other two are $\pm w$ where $w \neq \pm v$. If $w^2 = v^2 \pmod{N}$ but $w \neq \pm v \pmod{N}$, then $w^2 - v^2 = (w-v)(w+v) \equiv 0 \pmod{N}$, which means $N$ divides $(w-v)(w+v)$ but neither factor individually. Since $N = pq$, this allows us to recover the factors via $\gcd(N, w+v)$ and $\gcd(N, w-v)$. In this case, $\mathcal{B}$ can factor $N$.
\end{proof}
