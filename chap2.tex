...
\label{ssec:owf}
Consider password hashing: we want a function that's easy to compute (hash the password) but hard to invert (recover the password from the hash). This is exactly what one-way functions formalize. Cryptographers often attempt to base cryptographic results on conjectured computational assumptions to leverage reduced adversarial capabilities. Furthermore, the security of these constructions is no better than the assumptions they are based on. 
\begin{quote}
\emph{Cryptographers seldom sleep well.}\footnote{Quote by Silvio Micali in personal communication with Joe Kilian.}
\end{quote}
Thus, basing cryptographic tasks on the \emph{minimal} necessary assumptions is a key tenet in cryptography. Towards this goal, rather than making assumptions about specific computational problem in number theory, cryptographers often consider \emph{abstract primitives}. The existence of these abstract primitives can then be based on one or more computational problems in number theory.

The weakest abstract primitive cryptographers consider is one-way functions. Virtually, every cryptographic goal of interest is known to imply the existence of one-way functions. In other words, most cryptographic tasks would be impossible if the existence of one-way functions was ruled out. On the flip side, realizing cryptographic tasks from just one-way functions would be ideal. 
...
\begin{tikzpicture}[scale=0.9]
    % Left ellipse (domain) - yellow
    \draw[black, fill=yellow!50, line width=1.5pt] (0,0) ellipse (1.2cm and 2.2cm);
    \node[black, font=\small\bfseries] at (0,0) {$\{0,1\}^n$};
    
    % Right ellipse (range) - orange
    \draw[black, fill=orange!50, line width=1.5pt] (4,0) ellipse (1.2cm and 2.2cm);
    \node[black, font=\small\bfseries] at (4,0) {$\{0,1\}^m$};
    
    % Forward arrow (Easy to Compute) - top
    \draw[green!50!black, line width=2.5pt, ->, >=stealth] 
        (1.1,1.2) .. controls (1.8,1.5) and (2.2,1.5) .. (2.9,1.2);
    \node[green!50!black, font=\tiny\bfseries, above] at (2,1.6) {Easy to Compute};
    
    % Backward arrow (Hard to Invert) - bottom
    \draw[red!70!black, line width=2.5pt, ->, >=stealth, dashed, dash pattern=on 3pt off 2pt] 
        (2.9,-1.2) .. controls (2.2,-1.5) and (1.8,-1.5) .. (1.1,-1.2);
    \node[red!70!black, font=\tiny\bfseries, below] at (2,-1.6) {Hard to Invert};
\end{tikzpicture}
\caption{Visualizing One-way Functions}
\label{fig:owf}
\end{marginfigure}
...
We note that the function is not necessarily one-to-one. In other words, it is possible that $f(x) = f(x')$ for $x \neq x'$ -- and the adversary is allowed to output any such $x'$. For instance, if $f(x) = x \bmod 2$, then $f(0) = f(2) = 0$, and the adversary succeeds if it outputs either 0 or 2.
...
This condition is false for every function $f$. The point is that requiring perfect security (probability $0$) is too strong because an adversary $\mathcal{A}$ that outputs an arbitrarily fixed value $x_0$ succeeds with probability at least $1/2^{n}$, as $x_0 = x$ with at least the same probability. Even a trivial adversary that always outputs a fixed value succeeds with non-zero probability.
...
It is not known whether one-way functions exist. In fact, the existence of one-way functions would imply that $P \neq NP$ (see Exercise~\ref{ex:PNP}). This connection to complexity theory highlights the fundamental nature of one-way functions: they represent the boundary between what is efficiently computable and what is not. 
...
Intuitively, the answer is yes, we can specifically set $g(x_0) = y_0$, and otherwise have $g(x) = f(x)$.  In this case, the adversary gains the knowledge of how to invert $y_0$, but that will only happen with negligible probability, and so the function is still one-way. More formally, if an adversary could break $g$ with non-negligible probability, we could use it to break $f$ by handling the negligible case where $x = x_0$ separately.
...
The resolution of this apparent paradox is by noticing that a one-way function is only required to be one-way in the limit where $n$ grows very large.  So, no matter how many times we fix the values of $f$ to be 0, we are still only setting a finite number of $x$ values to 0.  However, this will still satisfy the definition of a one-way function - it is just that we will have to use larger and larger values of $n_0$ in order to prove that the probability of breaking the one-way function is negligible. This illustrates why cryptographic definitions are asymptotic: they allow for 'bad' behavior on finitely many inputs as long as security holds in the limit.

\subsection{Brittleness}
\paragraph{Example: OWFs do not always compose securely.}
Given a one-way function $f : \binfunc{n}{n}$, is the function $f^2(x) = f(f(x))$ also a one-way function?  Intuitively, it seems that if it is hard to invert $f(x)$, then it would be just as hard to invert $f(f(x))$.  One might try to invert $f$ by first inverting $f^2$ to get some $x'$ with $f^2(x') = f^2(x)$, then inverting $f$ on $f(x')$. But this fails because $f^2(x') = f^2(x)$ doesn't guarantee $f(x') = f(x)$ - we might have $f(x') \neq f(x)$ but $f(f(x')) = f(f(x))$.
% \sanjam{Explain the intuitive reduction that doesn't work.}  
Thus, this intuition is incorrect and highlights the delicacy when working with cryptographic assumptions and primitives. In particular, assuming one-way functions exists we describe a one-way function $f: \{0,1\}^{n}\times \{0,1\}^{n} \rightarrow \{0,1\}^{2n}$ such that $f^2$ can be efficiently inverted.
Let $g: \{0,1\}^n \rightarrow \{0,1\}^n$ be a one-way function then we set $f$ as follows:
$$f(x_1,x_2) = 0^{n}\|g(x_1)$$
Two observations follow:
\begin{enumerate} 
  \item $f^2$ is not one-way. This follows from the fact that for all inputs $x_1, x_2$ we have that $f^2(x_1,x_2) = 0^n || g(0^n)$. This function is clearly not one-way!
  \item $f$ is one-way. This can be argued as follows. Assume that there exists an adversary $\mathcal{A}$ such that $\mu_{\mathcal{A},f}(n) = \Pr_{x \stackrel{\$}{\leftarrow} \binset{n}}[ \mathcal{A}(1^{2n}, f(x)) \in f^{-1}(f(x))]$ is non-negligible. Using such an $\mathcal{A}$ we will describe a construction of adversary $\mathcal{B}$ such that $\allowbreak\mu_{\mathcal{B},g}(n) = \Pr_{x \stackrel{\$}{\leftarrow} \binset{n}}[ \mathcal{B}(1^n, g(x)) \in g^{-1}(g(x))]$ is also non-negligible. This would be a contradiction thus proving our claim.
      ...
      $$\mu_{\mathcal{B},g}(n) = \Pr_{x \stackrel{\$}{\leftarrow} \binset{n}}\left[ \mathcal{A}(1^{2n}, 0^n || g(x)) \in \{0,1\}^n || g^{-1}(g(x))\right].$$ (Here, the set $\{0,1\}^n || g^{-1}(g(x))$ means all strings that start with any $n$-bit prefix followed by a preimage of $g(x)$ under $g$.)
      But
      \begin{align*}
...
Below is another example of a transformation that does not work. One might think that dropping a single bit can't hurt security much, but we'll show that even this seemingly minor modification can completely break one-wayness. Given any one-way function $g$, let $g'(x)$ be $g(x)$ with the first bit omitted.
...
In this section, we show that even a very \emph{weak} form of one-way functions suffices for constructing one-way functions as defined previously. For this section, we refer to this previously defined notion as strong one-way functions. This is a powerful result: it shows that even if a function is only 'weakly' one-way (i.e., invertible with noticeable probability), we can amplify it to get a 'strong' one-way function (invertible with only negligible probability).
...
$$g(x_1, x_2, \cdots, x_q) = f(x_1) || f(x_2) || \cdots || f(x_q),$$
 is a strong one-way function. Let us discuss the intuition. A weak one-way function is "strong" in a small part of its domain. For this construction to result in a strong one-way function, we need just one of the $q$ instantiations to be in the part of the domain where our weak one-way function is strong. If we pick a large enough $q$, this is guaranteed to happen. More concretely, if the weak OWF is hard to invert on at least an $\alpha_f(n)$ fraction of inputs, then with $q$ independent copies, the probability that ALL are easy to invert is at most $(1-\alpha_f(n))^q$, which becomes negligible for large enough $q$.
...
Note that: (1) $\mathcal{A}(1^n, y)$ iterates at most $T = \frac{4n^2}{\alpha_f(n)\mu_{\mathcal{B},g}(nq)}$ times and each iteration runs in polynomial time. (2) $\mu_{\mathcal{B},g}(nq)$ is a non-negligible function. Since $\mu_{\mathcal{B},g}$ is non-negligible, there exists a noticeable function $\delta$ such that $\mu_{\mathcal{B},g}(nq) \geq \delta(nq)$ for infinitely many $n$. For such $n$, $T$ is polynomial. This implies that for infinite choices of $n$ the running time of $\mathcal{A}$ is bounded by a polynomial function in $n$.

It remains to show that $\Pr_{x \stackrel{\$}{\leftarrow} \binset{n}}[ \mathcal{A}(1^n, f(x)) = \bot] < \alpha_f(n)$ for arbitrarily large $n$. A natural way to argue this is by showing that at least one execution of $\mathcal{B}$ should suffice for inverting $f(x)$. However, the technical challenge in proving this formally is that these calls to $\mathcal{B}$ aren't independent. The calls share the same $x$ value, so if $\mathcal{B}$ fails on one call, it might be more likely to fail on others. We need to carefully analyze the dependency structure. Below we formalize this argument even when these calls aren't independent.\marginnote[-3cm]{\begin{lemma}
Let $A$ be any an efficient algorithm such that $\Pr_{x,r}[A(x,r) =1] \geq \epsilon$. Additionally, let $G = \{x\mid \Pr_{r}[A(x,r) =1] \geq \frac\epsilon2\}$. Then, we have $\Pr_x[x \in G] \geq \frac\epsilon2$.
\end{lemma}
\begin{proof}
...
$$S := \left\{x \left| \Pr_\mathcal{B}\left[\mathcal{A} \text{ inverts $f(x)$ in a single iteration} \right] \leq \frac{\alpha_f(n) \mu_{\mathcal{B},g}(nq)}{4n} \right. \right\}.$$ (Here, $\Pr_\mathcal{B}$ denotes the probability over the randomness of $\mathcal{B}$, which includes the random choice of $i$ and the random $x_j$ values.)
...
Assume, for the sake of contradiction,\marginnote[-8cm]{\begin{lemma}
...
This is interesting because unlike a typical cryptographic primitive that relies on a specific hardness assumption (which may or may not hold in the future), Levin's one-way function is future-proof in the sense that it will be secure as long as at least one hardness assumption holds (which we may or may not discover). This is a remarkable result: it shows that if OWFs exist at all, we can construct one explicitly without knowing which specific assumption to rely on.
...
\section{Hardness Concentrated Bit}
We start by asking the following question: Is it possible to concentrate the strength of a one-way function into one bit? In particular, given a one-way function $f$, does there exist one bit that can be computed efficiently from the input $x$, but is hard to compute given $f(x)$? This question is important because hard bits are useful for constructing pseudorandom generators and other cryptographic primitives. The answer is subtle: not every OWF has a hard bit, but we can always construct one that does.
...
Intuitively, the value $g(b, x)$ does not reveal any information about the first bit $b$, thus no information about the value $B(b, x)$ can be ascertained. Hence $\mathcal{A}$ cannot predict the first bit with a non-negligible advantage than a random guess. However, we are more interested in the case where the hard concentrate bit is hidden because of computational hardness and not information theoretic hardness. Information-theoretic security means the adversary literally has no information, while computational security means the adversary could theoretically compute it but it's computationally infeasible. The Goldreich-Levin theorem shows we can achieve computational hardness.
...
$$\Pr_{x,r}[E] =1.$$
We now construct $\mathcal{B}$ that inverts $g$ with perfect accuracy. While unrealistic, this case illustrates the key technique: using the hard bit to extract individual bits of $x$.
...
\end{marginfigure}
$$\Pr_{x,r}[E] \geq \frac{3}{4} + \epsilon(n),$$ where $\epsilon(\cdot)$ is non-negligible. The threshold of $3/4$ is chosen so that with two calls, we can use a union bound. With probability $\geq 3/4 + \epsilon$ of being correct, the probability that both calls are wrong is $\leq (1/4 - \epsilon)^2$, which is small enough for Chebyshev's inequality to work.
...
\begin{marginfigure}
    ...
    \end{marginfigure}
Very surprisingly, this idea along with careful analysis magically works out. The "magic" here is the use of pairwise independence: by guessing $\log T$ values and using XOR combinations, we get $T$ pairwise independent samples. This is a beautiful technique that appears in many cryptographic proofs.
