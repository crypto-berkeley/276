...
To reiterate, \textit{confidentiality} means that no eavesdropper that manages to get a hold of $c$ should be able to learn anything about $m$ that they do not already know (assuming they have no knowledge of the key $k$).
...
\begin{definition}[CCA Security]
    ...
    $$\advantage{\indcca}{\Pi,\adv} = \Big| \Pr[\pcnotionstyle{Priv\pcmathhyphen{}}\indcca_\Pi^\adv(\secpar) =1] -\frac12\Big|$$
    is a negligible function.
\end{definition}
...
Now we argue that 
$$|\Pr[\mb^{F_n(\cdot)}(1^n) = 1] - \Pr[\mb^{f(\cdot)}(1^n) = 1]|$$
is non-negligible.

\begin{align*}
    |\Pr[B^{F_n(\cdot)}(1^n) = 1] - \Pr[\mb^{f(\cdot)}(1^n) = 1]| &\ge \frac{1}{2} + \epsilon(n) - (\frac{1}{2} + \frac{q(n)}{2^n}) \\
...
One construction of a CPA-secure cipher is by the use of the counter mode.
...
\begin{definition}[Message Authentication Code Scheme]
...
\begin{definition}[Universal Hash Function]
    A function $h : \mathcal{F} \times \mathcal{F}^* \to \mathcal{F}$ (where $\mathcal{F}$ is a field of size $2^m$) is a universal hash function if for all $m, m' \in \mathcal{F}^{\le \ell}$ where $m \neq m'$ (i.e. $m$ and $m'$ have length at most $\ell$),
    \[
