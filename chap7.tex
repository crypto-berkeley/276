...
The PKG generates the Signing Key of Mike with $KeyGen(msk, id)$ and sends it back. 
...
\begin{definition}[Security of IBE]
    ...
    \[
    |\Pr[\text{Exp}_{\pi,\mathcal{A}}^{IBE,CPA}(n)=1]-\frac{1}{2}|=\text{negl}(n)
    \]
\end{definition}
...
First, we will give the hardness assumption in this scheme called \textbf{bilinear Diffie-Hellman} assumption, or BDH. 
...
\begin{definition}[Decisional bilinear Diffie-Hellman]
...
\begin{proof}
    ...
    \begin{enumerate}
        \item $\mathcal{B}$ receives 5 elements as input $\{g, g^{\alpha},g^{\beta},g^{\gamma},z\}$, where $\alpha,\beta,\gamma\stackrel{\$}{\gets}\mathbb{Z}_q,z\in\mathbb{G}$, and $\mathcal{B}$ need to determine whether $z=e(g,g)^{\alpha\beta\gamma}$ or not. 
...
\begin{theorem}
    Let $\mathcal{E}_{id}$ be a secure IBE with message space is super-poly. Then the derived signature scheme $\mathcal{S}$ is a secure digital signature scheme. 
\end{theorem}
\begin{proof}
    ...
    The IBE adversary $\mathcal{B}$ is modelled as follows:
    \begin{enumerate}
...
\begin{itemize}
    ...
    \item $\mathsf{Enc}(\sk = s \in \mathbb{Z}_q^{n+1}, m \in \mathbb{Z}_2) \rightarrow C \in \mathbb{Z}_q^{l \times (n+1)}$: Sample $A \gets \mathbb{Z}_q^{l \times n}$ and $e \gets \chi^{l}$. Define $B = A \| As' + e$ and $C = B + m \cdot G$ for a fixed gadget~\cite{EC:MicPei12} matrix $G \in \mathbb{Z}_q^{l \times (n+1)}$.
...
\begin{itemize}
    ...
    \item $\mathsf{Eval}(\times, C_1, C_2)$: Output $C = \mathsf{Flatten}(C_1) \times C_2$. 
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Output $pp = pk$ (public parameters) and $msk = sk$. 
    \end{enumerate}
